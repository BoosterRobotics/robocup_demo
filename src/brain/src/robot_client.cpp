...
int RobotClient::RLVisionKick(bool start)
{
    ...
    const std::string vkVersion = brain->config->get_rl_vision_kick_version();
    const auto vkEnum = (vkVersion == "kV1")
        ? booster::robot::b1::VisualKickVersion::kV1
        : booster::robot::b1::VisualKickVersion::kV2;  // Invalid values fall back to kV2.
    body["version"] = static_cast<int>(vkEnum);
    msg.body = body.dump();
